...
static void terminar_grupo(FILE *archivo, const Alerta *a, const char *tiempo) {
    ...
    if (kill(-a->pgid, SIGTERM) != 0) {
        ...
        snprintf(
            linea,
            sizeof(linea),
            "%s ERROR accion=SIGTERM_GRUPO pgid=%d señal=SIGTERM resultado=error errno=%d\n",
            tiempo,
            a->pgid,
            error_signal
        );
    ...
    snprintf(
        linea,
        sizeof(linea),
        "%s INFO accion=SIGTERM_GRUPO pgid=%d señal=SIGTERM resultado=enviada gracia_s=%d\n",
        tiempo,
        a->pgid,
        segundos_gracia
    );
    ...
    if (grupo_activo && puede_actuar(a, &motivo)) {
        if (kill(-a->pgid, SIGKILL) == 0) {
            snprintf(
                linea,
                sizeof(linea),
                "%s INFO accion=SIGKILL_GRUPO pgid=%d señal=SIGKILL resultado=enviada\n",
                tiempo,
                a->pgid
            );
        } else {
            ...
            snprintf(
                linea,
                sizeof(linea),
                "%s ERROR accion=SIGKILL_GRUPO pgid=%d señal=SIGKILL resultado=error errno=%d\n",
                tiempo,
                a->pgid,
                error_signal
            );
