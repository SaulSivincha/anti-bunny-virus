...
typedef struct { char tipo[32]; int pid; int ciclos; int nivel_emitido; } EstadoAlerta;
...
static EstadoAlerta *actualizar_persistencia(const char *tipo, int pid, int activo) {
    for (int i = 0; i < total_estados; ++i) {
        if (estados[i].pid == pid && strcmp(estados[i].tipo, tipo) == 0) {
...
static void agregar(Alerta *salida, int *total, int max, int pid, int pgid, unsigned int uid,
                    int accionable, const char *severidad, const char *tipo,
                    const char *descripcion, const char *evidencia) {
    if (*total >= max) return;
    Alerta *a = &salida[(*total)++];
    memset(a, 0, sizeof(*a));
    a->pid = pid; a->pgid = pgid; a->uid = uid; a->accionable = accionable;
...
int detectar(ProcesoInfo *procesos, int n_procesos, RecursoInfo *recursos, int n_recursos,
    ...
    for (int i = 0; i < n_procesos; ++i) {
        ...
        EstadoAlerta *estado = actualizar_persistencia("procesos", procesos[i].pid, activo);
        ...
        if (activo && estado != NULL && estado->nivel_emitido < nivel) {
            ...
            agregar(alertas_out, &total, max_alertas, procesos[i].pid, procesos[i].pgid, procesos[i].uid,
                    critica, critica ? "critica" : "sospechosa", "procesos",
    ...
    for (int i = 0; i < n_recursos; ++i) {
        ...
        EstadoAlerta *estado = actualizar_persistencia("recursos", recursos[i].pid, activo);
        int ciclos = estado != NULL ? estado->ciclos : 0;
        if (activo && estado != NULL && ciclos >= ciclos_persistencia && estado->nivel_emitido < 1) {
            ...
            agregar(alertas_out, &total, max_alertas, recursos[i].pid, -1, 0, 0, "sospechosa",
    ...
    for (int i = 0; i < n_archivos; ++i) {
        ...
        EstadoAlerta *estado = actualizar_persistencia("archivos", clave, activo);
        int ciclos = estado != NULL ? estado->ciclos : 0;
        ...
        if (activo && estado != NULL && ciclos >= ciclos_persistencia && estado->nivel_emitido < nivel) {
            ...
            agregar(alertas_out, &total, max_alertas, archivos[i].pid_escritor, archivos[i].pgid_escritor,
                    0, 0, correlacion ? "critica" : "sospechosa", "archivos",
